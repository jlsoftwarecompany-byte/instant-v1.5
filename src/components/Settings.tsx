      ...
      <main className="flex-1 max-w-2xl w-full mx-auto px-6 py-8 space-y-8 pb-20">
              ...
              <div className="grid grid-cols-5 gap-3">
                {LINKER_MASCOTS.map(m => (
                  <button
                    ...
                    style={selectedMascot === m.char ? {} : { backgroundColor: '#ffffff', borderColor: '#d4d4d8' }}
                    className={`
                      ...
                      ${selectedMascot === m.char
                        ...
                        : 'bg-white border-zinc-300 dark:bg-zinc-950 dark:border-zinc-800 dark:hover:border-purple-500/50'
                      }
                    `}
                    title={m.name}
                  >
                ...
                {LINKER_AURAS.map(a => (
                  <button
                    ...
                    className={`
                      ...
                      ${selectedAura === a.key
                        ? a.class
                        : 'bg-white dark:bg-zinc-950/40 border-zinc-300 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:text-pink-500 hover:border-pink-500/30 dark:hover:border-pink-500/20'
                      }
                    `}
                  >
                    <span>{a.label}</span>
        ...
        <section className="space-y-4">
          ...
          <div className="p-5 rounded-3xl theme-card border flex items-center justify-between bg-zinc-50/20 dark:bg-zinc-900/10">
            ...
            <button
              onClick={toggleTheme}
              className="relative h-8 w-16 rounded-full bg-zinc-200 dark:bg-zinc-950 border border-zinc-300 dark:border-purple-950 transition-colors focus:outline-none cursor-pointer"
            >
              <div
                className={`absolute top-1 inline-flex items-center justify-center h-6 w-6 rounded-full bg-white dark:bg-zinc-900 transition-all duration-200 shadow-md
                  ${theme === "black" ? "right-1" : "left-1"}
                `}
              >
                {theme === "black" ? (
